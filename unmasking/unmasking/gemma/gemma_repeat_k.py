...
class GemmaAttentionCustom(GemmaAttention):
...
class GemmaDecoderLayerCustom(GradientCheckpointingLayer):
    def __init__(self, config: GemmaConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = GemmaAttentionCustom(config=config, layer_idx=layer_idx)
...
class GemmaModelCustom(GemmaPreTrainedModel):
    def __init__(self, config: GemmaConfig):
        ...
        self.layers = nn.ModuleList(
            [
                GemmaDecoderLayerCustom(config, layer_idx)
...
class GemmaRepeatK(GemmaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.model = GemmaModelCustom(config)
    ...
    def forward(
        ...
        outputs: BaseModelOutputWithPast = self.model(
            input_idsr,
            attention_maskr=attention_maskr,
            position_ids=position_ids,
            past_key_values=past_key_values,
