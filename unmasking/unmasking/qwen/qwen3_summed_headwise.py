...
class Qwen3ModelCustom(Qwen3PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # print(unmask_conf)
        # u_list = torch.nn.functional.gumbel_softmax(
        #    self.unmasking_logits, tau=self.temp, hard=True, dim=-1
        # )
        # u_list.requires_grad = True
        # print(u_list)
        # with torch.no_grad():
        #    unmasking_probs = torch.nn.functional.softmax(self.unmasking_logits)
...
class Qwen3SummedHeadwise(Qwen3PreTrainedModel):
