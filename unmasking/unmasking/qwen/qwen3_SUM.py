...
class Qwen3AttentionCustom(Qwen3Attention):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.training:
            u = torch.rand((num_heads)).to(attn_output_unmasked.device)
            u = (u >= 0.5).int()
            unmask = u
            mask = 1 - u
            ...
            #    print(u)
            #    self.debug = 0
            attn_output_unmasked = attn_output_unmasked * u
            attn_output_masked = attn_output_masked * 0
...
class Qwen3SUM(Qwen3PreTrainedModel):
