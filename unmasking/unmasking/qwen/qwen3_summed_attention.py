...
def scaled_dot_product_attention(
    ...
    # if debug:
    #    print(attn_bias, attn_mask is not None)
    if enable_gqa:
...
class Qwen3AttentionCustom(Qwen3Attention):
    def __init__(self, config: Qwen3Config, layer_idx: int):
        ...
        self.debug = 0
        self.unmasking_logit = nn.Parameter(torch.zeros(1, 2))
        # self.unmasking_logit.requires_grad = True
        if layer_idx == 0:
            self.debug = 2
    ...
    def forward(
        ...
        # fix_u: Optional[torch.Tensor] = None,
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # u2 = torch.nn.functional.softmax(self.unmasking_logit, dim=-1)
        ...
        # attn_output = (1 / 2) * attn_output_unmasked + (1 / 2) * attn_output_masked
        # if self.debug:
        #    print("===")
        #    print(u2, self.unmasking_logit)
        #    print(self.unmasking_logit.requires_grad)
        #    print(u2.requires_grad)
        #    print("===")

        #    self.debug = 0

        attn_output = u2[0, 0] * attn_output_unmasked + u2[0, 1] * attn_output_masked
        if not self.training:
            with torch.no_grad():
                if self.unmasking_logit[0, 0] >= 0:
                    # print("Successfull unmasking.")
                    attn_output = attn_output_unmasked
                else:
...
class Qwen3ModelCustom(Qwen3PreTrainedModel):
    def __init__(self, config: Qwen3Config):
        ...
        # self.unmasking_logits = nn.Parameter(torch.zeros(config.num_hidden_layers, 2))
        # self.temp = 1.0
...
class Qwen3SummedAttention(Qwen3PreTrainedModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for z in range(sizes):
            unmask = [
                True if random.random() < p or (i >= 8 and i <= 15) else False
