# Standard library and external library import
import enum
...
def scaled_dot_product_attention(
    ...
    attn_bias = torch.ones((B, H, L, S), dtype=query.dtype, device=query.device)
    # print(attn_bias)
    if is_causal and attn_mask is None:
        ...
        attn_bias = attn_bias.masked_fill(~causal_mask, alpha)

    if attn_mask is not None:
        # Allow 2D (L, S), 3D (H, L, S), or 4D (B, H, L, S) masks
        if attn_mask.dtype == torch.bool:
            attn_bias = attn_bias.masked_fill(~attn_mask, alpha)
        else:
...
def scaled_dot_product_attention(
    ...
    # mask_lower = torch.tril(torch.ones((L, S), device=query.device))  # shape [L, S]

    # Upper triangle (strictly above diagonal) = 1
    mask_upper = torch.triu(torch.ones((L, S), device=query.device), diagonal=1)

    # Add batch and head dimensions
    # mask_lower = mask_lower.unsqueeze(0).unsqueeze(0)  # [1, 1, L, S]
    mask_upper = mask_upper.unsqueeze(0).unsqueeze(0)  # [1, 1, L, S]

    # Broadcast to attn_bias shape
    # mask_lower = mask_lower.expand(B, H, L, S)
    mask_upper = mask_upper.expand(B, H, L, S)

    # Construct attention bias
    # attn_bias = mask_lower + mask_upper * alpha
    # attn_bias = mask_upper * torch.log(torch.relu(alpha))
    attn_bias = mask_upper * torch.log(torch.sigmoid(50 * (alpha - 0.5)))
    # print(attn_bias)
    # print(alpha)
    # print(attn_bias)
    # if is_causal and attn_mask is None:
    ...
    attn_weight = torch.matmul(query, key.transpose(-2, -1)) * scale_factor
    if attn_weight.shape != attn_bias.shape:
        raise Exception("DIMENZIJEEE")

    # print(attn_weight.shape, attn_bias.shape)
    # print(attn_weight)
...
class Qwen3AlphaMasking(Qwen3PreTrainedModel):
    ...
    def initialize_alphas(self):
        ...
        with torch.no_grad():
            for idx, layer in enumerate(self.model.layers):
                if hasattr(layer, "self_attn") and hasattr(layer.self_attn, "alpha"):
                    # Use .data to modify the tensor in-place
                    layer.self_attn.alpha.data.fill_(0.5)
                    # print(
                    #    f"Set alpha on layer {idx} to: {layer.self_attn.alpha.item()}"
                    # )
                    # print(layer.self_attn.alpha.requires_grad)
        # print("Finished initializing alphas.")
