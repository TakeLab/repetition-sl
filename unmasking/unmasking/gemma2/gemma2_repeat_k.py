...
class Gemma2RepeatK(Gemma2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.model = Gemma2ModelCustom(config)
        self.k_repeat = config.k_repeat + 1
    ...
    def forward(
        ...
        input_idsr = torch.cat([input_ids] * self.k_repeat, dim=1)
        attention_maskr = torch.cat([attention_mask] * self.k_repeat, dim=1)
        outputs: BaseModelOutputWithPast = self.model(
            input_idsr,
            attention_mask=attention_maskr,
            position_ids=position_ids,
            past_key_values=past_key_values,
        ...
        sequence_output = outputs.last_hidden_state[:, -seq_len:]
